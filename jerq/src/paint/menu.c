...
void Oread(){
	picread(getname(picdir));
}
void Owrite(){
	iwrite(canvas, getname(picdir));
}
void Onegimage(){
	bitblt(canvas->alf, canvas->rect, canvas->grn, canvas->rect.or, F_XOR);
}
void Obread(){
	brushread(getname(brushdir));
}
int getbrname=1;
void Obwrite(){
	strcpy(brushname, getname(brushdir));
	iwrite(brush, brushname);
	getbrname=1;
}
void Otread(){
	rdtexture(&tex, getname(texdir));
}
int gettexname=1;
void Otwrite(){
	wrtexture(&tex, getname(texdir));
	gettexname=1;
}
void Onegtexture(){
	register i;
