...
main()
{
	...
	if(scbf[0]=='F') {
		def_font = 1;
		for(j=0;j<MXFONT;j++) fontrcv[j] = font_notav[j] = 0;
	}
	else def_font = 0;
	jtag(1,"Version 3 ");	/*that initializes placelab*/
	...
	for(;;sleep(2),wait(MOUSE)){
		...
		if(button1() ) {
			...
			if(MX < board.origin.x){ /*mouse on menu*/
				release1();
				subsel = wheremouse(m_size);
				if(vault[c_menu].imd>1) vault[c_menu].fun();
				else {
					if(vault[c_menu].imd &&
