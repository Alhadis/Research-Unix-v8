...
short mice_pts() {
	...
	*pbfp++ = q;
	...
	nn = pbfp - pbuf;
	if(nn>1) {
		bb = mk_obj(pbuf,(char *)0,'p',SOLID,nn,(short)' ',(short)0);
		if(bb) {
...
short draw_spec(kind){
	...
	for(;;sleep(2),wait(MOUSE)){
		...
		case 0:	/*text*/
			...
			if((cpn-2)>story)
			bb = 
			mk_obj(&cp0,story,'t',ROMAN,(short)1,def_size,(short)0);
			else bb = 0;
		...
		case 1:	/*circle*/
			...
			if(rubber(cp,&pq)) {
				...
				if(rad < 2) rad = 2;
				bb =
				mk_obj(&cp,(char *)0,'c',SOLID,1,rad,0);
			}
			else bb = 0;
			bmark(cp,poscross);
			goto f01;
...
short draw_box(oval)
{
	...
	bb = mk_obj(cp,(char *)0,'P',SOLID,(short)4,(short)' ',(short)0);
	if(bb) {
		obj[no_obj++] = bb;
		plot_one(bb);
