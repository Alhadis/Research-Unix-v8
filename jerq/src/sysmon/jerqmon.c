...
#define TIMEORIGIN 2
#define TIMECORNER (NAMELEN*SWIDTH)
#define LOADORIGIN TIMECORNER
#define LOADCORNER (TIMECORNER + LOADLEN*SWIDTH)
#define MAILORIGIN LOADCORNER
#define MAILCORNER (LOADCORNER + MAILLEN*SWIDTH)
int size;
int charheight = 12;
int barheight = MAXBARHEIGHT;
int iconorigin = MAILCORNER;
...
/* dynamicly add/remove monitored systems */
...
int ind;				/* index into node array */
{
	...
	if (ind != 0 && size >= TIMECORNER)
		string (&defont, n->name, &display, 
			Pt(TIMEORIGIN+Drect.origin.x, n->bar.origin.y+barheight-4),
			F_XOR);
};
	...
	struct nodedef *n;
{
	...
	string(&defont, n->load, &display, 
		Pt (Drect.origin.x+LOADORIGIN, n->bar.origin.y+barheight-4), F_XOR);
}
...
drawtime()
{
	...
	string(&defont, time, &display,
		Pt (Drect.origin.x+TIMEORIGIN, Drect.origin.y + barheight), F_XOR);
}
	...
	struct nodedef *n;
{
	...
	string(&defont, n->mail, &display,
		Pt (Drect.origin.x+MAILORIGIN, n->bar.origin.y+barheight-4), F_XOR);
}
