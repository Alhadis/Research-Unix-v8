		...
		case '\033':
			...
			case '@':	/* insert character */
				...
				bitblt(p->layer, Rpt(pt(up->x, up->y),
						     pt(up->xmax, up->y+1)),
				       p->layer, pt(up->x+1, up->y), F_STORE);
				string(&defont,buf,p->layer,pt(up->x,up->y),F_STORE);
				up->x++;
				break;
...
register struct User *up;
{
	rectf(P->layer, Rpt(sub(pt(up->x, up->y), Pt(1,1)),
			    add(pt(up->x, up->y), Pt(9,14))), F_XOR);
	if (bl==0)
		return;
	rectf(P->layer, Rpt(pt(up->x, up->y),
		    add(pt(up->x, up->y), Pt(8,13))), F_XOR);
}
...
/*
 * Rchar behaves much like rcvchar, but interposes
