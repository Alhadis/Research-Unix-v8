	...
	if( stp == UNDEF|| stp == FARG ){
		...
		default:
			if(!(class&FIELD)) uerror(
				"declared argument %.8s is missing", p->sname );
		case MOS:
		case STNAME:
		case MOU:
		case UNAME:
...
NODE *
rstruct( idn, soru ){ /* reference to a struct or union, with no definition */
	register struct symtab *p;
	register NODE *q;
	p = &stab[idn];
	switch( p->stype ){
...
# endif
	...
	for( i = oparam+4;  i< paramno; ++i ){
		...
		if( sz == 0 ){
			werror( "structure member has size 0: %.8s", p->sname );
			}
		if( sz > strucoff ) strucoff = sz;  /* for use with unions */
		SETOFF( al, sa );
		/* al, the alignment, is the LCM of the members' alignments */
		}
	dstash( -1 );  /* endmarker */
	SETOFF( strucoff, al );
...
# endif
		...
		pstk->in_off =  off;   /* offset at the beg. of this element */
		...
		if( (iclass==AUTO||iclass==REGISTER) && (ISARY(t) || t==STRTY))
...
# ifndef NODBG
			if (ddebug) printf(
				"removing %8s = stab[ %d], flags %o level %d\n",
