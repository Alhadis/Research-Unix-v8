...
input()
{
	...
	for (;;) {
		...
		case JSW:
			...
			p->op = oper&0377;
			p->subop = oper>>8;
			if (*curlp=='L' && *(curlp+1)=='%' && (p->labno = getnum(curlp+2)))
				p->code = 0;
			else {
				p->labno = 0;
				p->code = copy(1, curlp);
...
output()
{
	...
	default:
		...
		if (t->code) {
			...
			if( t->ref && 
			    abs(t->refc-t->ref->refc) < MAXSHORT && notdot(t)) {
				printf(".b\tL%%%d\n", t->labno);
			} else {
				printf("\tL%%%d\n", t->labno);
			}
...
refcount()
{
	...
	for (p = first.forw; p!=0; p = p->forw) {
		...
		} else if (p->op==SHORT || p->op==PCRELO || p->op==PCRELOX
			 || p->op==STABS) {
			int l1;
			char *cp = p->code;
			extern char *strrchr();
...
iterate()
{
	...
	for (p = first.forw; p!=0; p = p->forw) {
		...
		if (p->op==JBR || p->op==JMP || p->op==CBR) {
			...
			rp = p->forw;
			while (rp && (rp->op==LABEL || rp->op==STABS)) {
				if (p->ref == rp) {
					p->back->forw = p->forw;
					p->forw->back = p->back;
					p = p->back;
...
struct node *p;
{
	...
	if( p->code && (cp = index(p->code, '%')) && *++cp=='p' && *++cp=='c' )
		return(1);
	return(0);
}
...
struct node *t;
{
	...
	if( t->refc < (xrefc = t->ref->refc) ) {
		for(t=t->forw;t->refc != xrefc; t=t->forw) {
			switch( t->op ) {
			case LABEL:
			case DLABEL:
			case TEXT:
