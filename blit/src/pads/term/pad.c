...
PadClip()
{
	...
	for( p = Sentinel.back; ISPAD(p); p = p->back ){
		...
		if( !PadSized(p->rect) ) p->rect = ZRectangle;
	}
...
register long obj;
{
	...
	PutRemote( P_KBDSTR );
	SendLong( Current->object );	/* Current != 0; */
	assert(obj);
	SendLong(obj);
	SendString(kbds);
	kbds[0] = '\001';
...
long LiveKBD()
{
	...
	if( sel && (sel->attributes&ACCEPT_KBD) ){
		...
		return assert( sel->object );
	}
	if( cur && (cur->attributes&ACCEPT_KBD) ){
		HeavyBorder(cur);
		return assert( cur->object );
	}
	return 0L;
}
...
{
	register char c, *t;
	register long live, lenmake, len;
