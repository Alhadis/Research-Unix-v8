...
struct mxrec mxrecs[MSIZE];
int mxx;

mxtrace(gp, cp, a, b, c, d)
struct group *gp;
struct chan *cp;
{
register struct mxrec *mp;
register x;
	x = mxx;
	mp = &mxrecs[x++];
	if (x >= MSIZE)
	...
	mxx = x;
	if (gp)
		mp->m_g = gp->g_state; else
		mp->m_g = 0;
	if (cp)
		mp->m_index = cp->c_index; else
		mp->m_index = -1;
	mp->m_a = a;
	mp->m_b = b;
	mp->m_c = c;
...
out:
	...
	if (ip->i_count == 1) {
		...
		ip->i_un.i_rdev = 0;
		ip->i_flag |= IUPD|ICHG;
		mxtrace(0,0,MXIPUT,ip,ip->i_flag,ip->i_mode);
		iput(ip);
		return;
	}
	prele(ip);
...
UDCL
{
	...
	if (fmp != FMP) {
		if (U(u_count) == 0)
			return;
		msread(fmp, FP->f_un.f_chan UIO);
		return;
	}

	mxtrace(gp, 0, MXREAD, gp->g_datq, 0, 0);

	if ((fcount=U(u_count)) == 0) {
		spl6();
		if (gp->g_datq == 0)
			u.u_error = ENXIO;
		splx(s);
	...
	while (fcount==U(u_count)) {
		...
		while (gp->g_datq && u.u_count >= CNTLSIZ + 2) {
			...
			if (markcp==NULL)
				markcp = cp;
			else if (cp==markcp && cp->c_flags&FBLOCK) {
				mxtrace(gp, cp, MXFSLEEP, u.u_count, 0, 0);
				if(fcount!=U(u_count))
					break;		/* will drop out and return */
				sleep((caddr_t)&gp->g_datq,TTIPRI);
				markcp = NULL;
				continue;
			}
			if (cp->c_flags&FBLOCK) {
				mxtrace(gp, cp, XSKIP, 0, 0, 0);
				continue;
			}
			h.index = cpx(cp);
			...
			if (esc) {
				...
				if (gp->g_state&AUTOBLK && !(cp->c_flags&PORT)) {
					mxtrace(gp, cp, SETFBLOCK, 0, 0, 0);
					cp->c_flags |= FBLOCK;
				}
			}
			if (U(u_count) && (xfr&1)) {
			...
			mxtrace(gp, cp, MXRCOPY, h.count, h.ccount, cp);
...
caddr_t	ubase, hbase;
	...
	mxtrace(gp, 0, MXWRITE, gp->g_datq, u.u_count, 0);
	burpcount = 0;
	while (u.u_count >= sizeof h) {
		...
		esc = 0;
		if (h.count==0) {
			if (h.count=h.ccount)		/* permit zero writes */
				esc++;
		}
		cp = xcp(gp, h.index);
...
int s;
	...
	mxtrace(0, cp, SLAVEGET, u.u_count, 0, 0);
	s = spl6();
	while (q->c_cc == 0) {
		if (cp->c_flags&WCLOSE) {
			u.u_error = ENXIO;
...
register struct chan *cp;
{
	...
	mxtrace(0, cp, SLAVEPUT, u.u_count, cp, fmp&FMPX);
	while (u.u_count) {
		(void) spl6();
		if (cp->c_flags&WCLOSE) {
...
int s;
	...
	if (cp->c_flags&b && q->c_cc<LOQ) {
		cp->c_flags &= ~b;
		if (b&ALT) {
			mxtrace(0, cp, MXRSTRT, 0, 1, 0, 0);
			wakeup((caddr_t)q+1);
		} else {
			mxtrace(0, cp, MXRSTRT, 0, 2, 0, 0);
...
int	s;
	...
	case M_IOCTL:
		break;
	case M_IOANS:
		mxtrace(0, cp, MXIOANS, cp->c_flags, 0, 0);
...
struct file *fp;
	...
	if (fmp == FMP) {
		...
		mxsioctl(fp->f_un.f_chan, cmd, addr, flag);
	}
}
...
struct {
	...
	mxtrace(cp->c_group, cp, MXIOCTL, cmd, 0, 0);
	ctlbuf.c_cmd = cmd;
	(void) copyin(addr, (caddr_t)&ctlbuf.c_vec, sizeof (struct sgttyb));
	sioctl(cp, (char *)&ctlbuf, sizeof ctlbuf);
...
register i;
	...
	gp = cp->c_group;
	mxtrace(gp, cp, MXCHFREE, cp->c_flags&PORT, cp, 0);
	if (gp==NULL)
		return;
	i = cp->c_index;
	if (cp == gp->g_chans[i]) {
		gp->g_chans[i] = NULL;
		mxtrace(gp, cp, MXCHFREE2, 0, 0, 0);
	}
	cp->c_group = NULL;
	wakeup((caddr_t)gp);
}
...
register struct clist *q;
	...
	while (q->c_cc) {
		cp->c_flags |= BLOCK;
		mxtrace(0, cp, MXSIOCTL, q->c_cc, 1, 0);
		if (sdata(cp)==NULL) {
			u.u_error = ENXIO;
			return;
		}
		sleep((caddr_t)q+1, TTOPRI);
		mxtrace(0, cp, MXSIOCTL, q->c_cc, 2, 0);
	}
	(void) b_to_q(vec, cc, &cp->c_ctlx);
	cp->c_flags |= SIOCTL;
	while (cp->c_flags&SIOCTL) {
		if (cp->c_ctlx.c_cc) {
			mxtrace(0, cp, MXSIOCTL, cp->c_ctlx.c_cc, 3, 0);
			if (sdata(cp)==NULL) {
				u.u_error = ENXIO;
				return;
			}
		}
		mxtrace(0, cp, MXSIOCTL, 0, 4, 0);
		sleep((caddr_t)cp, TTOPRI);
		mxtrace(0, cp, MXSIOCTL, 0, 5, 0);
	}
