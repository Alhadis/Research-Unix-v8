...
Reshape(){
	...
	r=getrect();
	if(r.corner.x-r.origin.x>100 && r.corner.y-r.origin.y>50){
		Point save;
		save=l->rect.origin;
		dellayer(l);
		p->state&=~MOVED;
...
{
	if(kbdlistp<=kbdlist || sendbusy || (OUTQUEUE.c_cc >= (CBSIZE/2)))
		return;	/* avoid "sw" in "sendpkt" */
	if(psend((int)(kbdproc-proctab), kbdlist,
		    (int)(kbdlistp-kbdlist), C_SENDNCHARS)!=-1)
			kbdlistp=kbdlist;
}
mpxdelwind(w){
