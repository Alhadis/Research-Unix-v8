...
/*
 * The following three manifests define the sizes of the chunks in
 * which the loader handles file I/O.
 */
#define LDBSHIFT 12
#define LDBSIZE  (1<<LDBSHIFT)
#define LDBMASK  (LDBSIZE-1)
...
top:
	...
	if (n > BUFSIZ) {
		take = n - n % LDBSIZE;
		lseek(infil, (sp->bno+1)*LDBSIZE, 0);
		...
		sp->size -= take;
		sp->pos += take;
		dseek(sp, (sp->bno+1+take/LDBSIZE)*LDBSIZE, -1);
		goto top;
	}
	*loc++ = get(sp);
	--n;
		...
		if (p->nuser==0 || (p = &page[0])->nuser==0) {
			if (page[0].nuser==0 && page[1].nuser==0)
				if (page[0].bno < page[1].bno)
					p = &page[0];
			p->bno = b;
			lseek(infil, loc & ~(long)LDBMASK, 0);
...
STREAM *asp;
{
	...
	if ((sp->nibuf -= sizeof(char)) < 0) {
		dseek(sp, ((long)(sp->bno+1)<<LDBSHIFT), (long)-1);
