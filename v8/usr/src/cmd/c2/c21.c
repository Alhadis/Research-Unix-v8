...
ashadd:
	...
	case EXTZV:
	{
		...
		**	extv	$n*16,$16,A,B	>	cvtwl	(n/2)+A,B
		**	extzv	$n*8,$8,A,B	>	movzbl	n+A,B
		**	extzv	$n*16,$16,A,B	>	movzwl	(n/2)+A,B
		*/
		...
		if (flen == 8 || flen == 16)
		{
			...
			register int	coff;	/* chunk (byte) offset*/

			if (regs[RT1][0] != '$') goto std;
			if ((boff = getnum(&regs[RT1][1])) < 0) goto std;
			coff = boff / 8;
			if (coff) {
				if(isreg(regs[RT3]) >= 0) goto std;
				if(!addable(regs[RT3])) goto std;
			}
...
register struct node *p;
{
	...
	|| ((p->op==CVT || p->op==MOVZ)
		...
		if (regs[r][1]!=0) {/* send directly to destination */
			...
			lastrand = byondrd(p);
			goto buggery;	/* skip check for writing dead reg */
		...
		if ((p->subop&0xF)==LONG && *(regs[RT2])!='$') {/* result of EXTZV is long */
			...
			if (siz==8 && pos==0) {
				...
				if (equstr(src, lastrand)) {
					p->subop = U(LONG, OP2);
					sprintf(line, "%s,%s", regs[RT1],src);
				} else {
					p->subop = U(LONG, OP3);
					sprintf(line, "%s,%s,%s",
					       regs[RT1],src,lastrand);
				}
			}
			p->pop=0;
...
/* can address A be transformed to n+A ?  */
addable(p)
register char *p;
{
	if (isalnum(*p) || *p=='_')
