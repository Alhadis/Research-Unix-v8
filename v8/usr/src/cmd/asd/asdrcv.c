 ...
 *	following pipeline:
 *
 *		unseal -K /etc/asd/keys/machine/uid | inspkg
 */
	...
	char **argv;
{
	...
	/* each argument must be nonempty and all letters or digits */
	for (i = 1; i < argc; i++) {
	...
	default:			/* parent */
		...
		close (pipefd[0]);
		execl ("/bin/inspkg", "inspkg", (char *) 0);
		execl ("/usr/bin/inspkg", "inspkg", (char *) 0);
