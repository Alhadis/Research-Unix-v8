...
ccwarn:
	...
	if (o == CBRANCH) switch (BTYPE(l->in.type)) {
	case STRTY:
	case UNIONTY:
		if (!ISPTR(l->in.type))
			uerror("struct or union as conditional");
	case ENUMTY:
		break;
	}
...
# endif
	...
	if( actions & OTHER )
	{
		...
		case STREF:
			...
			{
				uerror( "%s not structure/union member",
					sp->sname);
				break;
			}
			else
...
# endif
	if( (k = j) < 0 ) uerror( "undefined structure or union %s", stab[i].sname);
	else
	{
		for( ; (kk = dimtab[k] ) >= 0; ++k )
		{
