...
u_long addr, myinaddr;
{
	...
	fprintf(stderr, "arpwhohas(%2x%2x%2x%2x%2x%2x %x %x)\n",
		myenaddr[0], myenaddr[1], myenaddr[2], myenaddr[3],
		myenaddr[4], myenaddr[5], myinaddr, addr);
...
struct ether_arp *ap;
{
	...
	if(ap->arp_op == ntohs(ARPOP_REPLY)){
		...
		if(ioctl(ipfd, IPIORESOLVE, &goo) < 0)
			perror("IPIORESOLVE");
		fprintf(stderr, "\tresolving: %2x%2x%2x%2x%2x%2x %x\n",
		  goo.enaddr[0], goo.enaddr[1], goo.enaddr[2], goo.enaddr[3],
		  goo.enaddr[4], goo.enaddr[5], goo.inaddr);
		return;
	}
reply:
	if(tpa != myinaddr) {
		fprintf(stderr, "\trequest: not me\n");
		return;
	}
