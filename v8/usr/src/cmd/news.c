...
/* The maximum number of spaces to keep together, without trying to tab */
#define MAXSP 2
	...
	char *f;
{
	...
	if ((fd = fopen (fname, "r")) == NULL) {
		...
		while ((c = getc (fd)) != EOF) {
			...
			default:
				/*
				 *	emit characters to get the output
				 *	to the position that corresponds
				 *	to this input character.
				 *
				 *	first, backspace if necessary
				 */
				while (ip < op) {
					putchar ('\b');
					op--;
				...
				/*
				 *	send tabs to get close to the right
				 *	place.  arrange to leave suitably
				 *	small number of spaces as such.
				 */
				if (ip - op > MAXSP) {
					while ((op & -8) < (ip & -8)) {
						putchar ('\t');
