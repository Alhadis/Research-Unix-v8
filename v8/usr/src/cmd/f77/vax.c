...
#	ifdef UCBVAXASM
	...
	sprintf(buff, "\t.stabd\t0%o,0,0%o", code, type);
	return(buff);
	}
#	endif
...
sprintf(t, "0%o,0,0%o,%s", code,  type, (loc? loc : "0") );
return(buff);
}
...
ftnint leng;
{
#if NOTDEF		/* not useful at the moment */
ftnint iarrlen();
...
if(p->vdim)
	...
	type = (type << OF) | FTN;
return ((type << BASIC) | types2[p->vtype]);
}
...
register struct Extsym *xp;
{
	...
	prstab(varstr(VL,np->varname), N_SSYM, stabtype(np), nbuf);
}
#endif
