  ...
  char **argv;
{
  ...
  if(j>100) fprintf(stderr,"using %dkB workspace\n",j);
  ...
  if(rdframe(here,in,mid)==0) error("unexpected empty first frame");
  if(detrend){
    fit(here->p,d);
    rmfit(here->p,d);
  }
  ...
  if(tinterp&&(rd.nfr>1)){
    ...
    while( ifr<=nfr ){
      ...
      t = (twtime-here->time)/((there->time-here->time)+1e-20);
      if(verbose) fprintf(stderr,"twtime=%g t=%g\n",twtime,t);
      if( (t<0) || (t>1) ){
...
/*----old version-----
    while( rdframe(there,in,mid) ){
      if(detrend) rmfit(there->p,d);
      wrframe(there->time,there->p,timebar);
      tframe = there; there = here; here = tframe;
    }
*/
    while( rdframe(here,in,mid) ){
      if(detrend) rmfit(here->p,d);
      wrframe(here->time,here->p,timebar);
    }
  }
