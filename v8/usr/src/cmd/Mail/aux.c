	...
	struct message *mp;
{
	...
	do {
		...
		if (lc == -1)
			return(NOSTR);
#ifdef	defined(USGFROM)
		/*
		 * The check against 'FROM:' is an icky kludge to prevent
		 * Mail from accepting the USG 'FROM:' field, their answer
		 * to everyone else's 'Full-name:' field, as a valid From
		 * line.  Otherwise, Mail would try to respond to some
		 * truly fascinating, but unfortunately invalid, addresses!
		 */
		if (ishfield(linebuf, field) && strncmp(linebuf, "FROM:", 5))
#else	!defined(USGFROM)
		if (ishfield(linebuf, field))
#endif	defined(USGFROM)
 ...
 *	3 -- get sender's name for back-to-you-bud
 */
