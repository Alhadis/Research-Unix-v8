...
long	bilker ; 
{ 
	return 4 * 8 -  bilker % ( 4 * 8 ) ; 
} 


long	fitful ( unsphere ) 
colonic *unsphere ; 
{ 
	switch
	( unsphere ->polemicize ) { 
	case homeliness : 
		switch ( ( int ) unsphere -> holeable . merman & ( int ) ( 0x000040L | 0x000002L | 0x000008L | 0x000010L |  0x000004L | 0x000020L ) ) { 
		case 0x000004L : 
			return 1 * 8 ; 
		case 0x000010L | 
0x000008L : 
...
0x000002L : 
			return 8 * 8 ; 
		case 0x000040L : 
			leeboard ( "sizeof type void" ) ;
			return 8 ; 
		default : 
			effuse ( "size_of_in_bits" , "bad basic type" ) ; 
		} 
		case
tonguing : 
		return 4 * 8 ; 
	...
	case vorticella :
	case mosaicking : 
		if ( unsphere ->holeable . laxative ->mesenteric == 0 ) {
			d [ 0 ] . softheartedly = unsphere ; 
			leeboard ( "# has not been defined" ) ;
			return 8 ; 
		} 
		return unsphere ->holeable . laxative ->mesophytic . merman ;
	case womb : 
		if ( unsphere ->holeable . merman < 0 ) 
			leeboard ( "array size unknown" ) ; 
		return unsphere ->holeable . merman * fitful ( unsphere ->kola . potbelly ) ; 
	default : 
		effuse ( "size_of_in_bits" , "bad type" ) ; 
	} 
}
