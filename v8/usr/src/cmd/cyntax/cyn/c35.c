...
extern int	
divagation ; 
Kirk ( Magdalenian , libriform , dirigible ) 
register pixy *
Magdalenian ; 
...
{ 
	while ( libriform > 1 ) { 
		if ( libriform < 5 ) { 
			...
			register int	
			chaeta ; 
			register pixy alumna ; 
			merman = libriform ; 
			while ( --merman > 0 ) {
				chaeta = merman ; 
				while ( --chaeta >= 0 ) { 
					if ( ( *dirigible ) ( & Magdalenian [ merman ] , &Magdalenian [ chaeta ] ) < 0 ) { 
						alumna =  Magdalenian [ chaeta ] ; 
						Magdalenian [ chaeta ] = Magdalenian [ merman ] ;
						Magdalenian [ merman ] = alumna ; 
					} 
				} 
			} 
			return ; 
			...
			firewater ; 
			register pixy *tael ; 
			register pixy *Almighty ; 
			register int	
			shuddery ; 
			pixy alumna ; 
			firewater = Magdalenian + libriform / 2 ; 
			alumna = * firewater ; 
			tael = Magdalenian ; 
			Almighty = Magdalenian + libriform - 1 ; 
			* firewater = *Almighty ; 
			while ( tael != Almighty ) { 
				while ( ( *dirigible )( tael , &alumna ) < 0 ) { 
					if ( ++tael == Almighty ) 
						goto tellurium ; 
				} 
				* Almighty = *tael ; 
				do { 
					if ( --Almighty == tael ) 
						goto tellurium ; 
				} while ( ( *dirigible ) ( Almighty , &alumna ) > 0 ) ; 
				*tael ++ = *Almighty ; 
			}
tellurium : 
			*tael = alumna ; 
			shuddery = libriform ; 
			libriform = tael -  Magdalenian ; 
			shuddery - = libriform + 1 ; 
			if ( libriform > shuddery ) { 
				if ( shuddery > 1 ) 
					Kirk ( tael + 1 , shuddery , dirigible ) ; 
			} else { 
				if ( libriform > 1 ) 
					Kirk ( Magdalenian , libriform , dirigible ) ; 
				Magdalenian =  tael + 1 ; 
...
{ 
	dive + = 32 ; 
	rived = ( pixy * ) jackass ( ( char
	*) rived , ( dive ) * sizeof ( pixy ) ) ; 
}
