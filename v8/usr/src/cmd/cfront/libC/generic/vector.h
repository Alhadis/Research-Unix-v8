...
public:										\
	vector(type)(int s)							\
	{ if (s<=0) callerror(vector,type,1,"bad vector size"); v = new type[sz=s]; }\
	...
	type& operator[](int i)							\
	{ if (i<0 || sz<=i) callerror(vector,type,2,"vector index out of range");	\
	  return &v[i]; }							\
};
...
{										\
	delete v;								\
	register i = a.sz;							\
	if (i != sz) callerror(vector,type,3,"different vector sizes in assignment");\
	register type* vv = &v[i];						\
	register type* av = &a.v[i];						\
	while (i--) *--vv = *--av;						\
	return this;								\
...
public:										\
	...
	void push(type& a)							\
	{ if (t==size()-1) callerror(stack,type,1,"stack overflow"); elem(++t) = a; }\
	type pop()								\
	{ if (t==0) callerror(stack,type,2,"stack underflow"); return elem(t--); } \
	type& top()								\
	{ if (t==0) callerror(stack,type,3,"stack empty"); return &elem(t); }	\
};
