...
//error('d',"deref %t",t?t:t1);
		...
		if (t) {	/*	*t	*/
			...
			if (t1->vec_type()) {
				t2->integral(b);
				tp = t1->deref();
				if (e1->base == DEREF && e1->e2!=zero) {
						// (*p)[i] or p[j][i]
						// sizeof(p) must be known
					Ptype t = e1->tp;
					switch (t->base) {
					case VEC: if (Pvec(t)->size) break;
					case PTR: error('w',"vector size unknown: p[i][j] means *(*(p+i)+j)");
					}
				}
			}
...
//error('d',"%t=%t %d %d",t1,t2,c1,c2);
				if (c1 != c2) {
					...
					*this = *e2;
					...
					if (cl->itor/*cl->has_ctor()*/ && cl->has_dtor())
...
/*error('d',"dot %d %d",e2->mem->tp,e2->mem->tp?e2->mem->tp->base:0);*/
		...
		{	Pname cn;
			...
			&& (i=can_coerce(t1,t2))
			&& Ncoerce) {
				if (1 < i) error("%d possible conversions for assignment");
//error('d',"%t =%t",t1,t2);
				Pclass cl = (Pclass)cn->tp;
				Pref r = new ref(DOT,e2,Ncoerce);
				Pexpr c = new expr(G_CALL,r,0);
