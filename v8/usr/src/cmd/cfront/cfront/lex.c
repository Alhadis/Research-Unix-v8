...
static char* file_name[MAXFILE];	/* stack of source file names */
		...
		case '"':
			...
			case '"':
				...
				if (*txtstart) {
					...
					char* fn;
					if (f == 0) goto push;
					if ( (fn=file_name[f]) && (strcmp(txtstart,fn)==0) ) {
						/* same file: ignore */
					}
					else if ( (fn=file_name[f-1]) && (strcmp(txtstart,fn)==0) ) {
						/* previous file: pop */
					/*	delete(file_name[f]);*/
						curloc.file--;
					}
				...
				push:
						...
						if (MAXFILE<=++f) error('i',"fileN buffer overflow");
