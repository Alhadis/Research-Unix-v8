...
//error('d',"a%n cln%n init %k",a,cln,init->base);
	...
	inin2:
		...
		init = init->typ(tbl);
		init = class_init(a,a->tp,init,tbl);
		init->simpl();
		init->permanent = 2;
		a->n_initializer = init;
	}
		...
		/* ????: */		|| (i=can_coerce(a->tp,init->tp))) {
						if (1 < i) error("%d possible conversions for defaultA");
						if (Ncoerce) {
...
/*error('d',"n1%n n2%n\n",n1,n2);*/
			...
			bdbd: 
				...
				for (a1=f->argtype, a2=nf->argtype; a1; a1=a1->n_list, a2=a2->n_list) {
					int i1  =  a1->n_initializer
						|| a1->n_evaluated;
					int i2  =  a2->n_initializer
						|| a2->n_evaluated;
					if (i1) {
						if (i2
						&& (	a1->n_evaluated==0
							...
							error("twoIrs for%nA%n",nn,a2->string?a2:a1);
					}
					else if (i2) {
...
//error('d',"c/dtor %n %d %d",cn,ctor,dtor);
			if (dtor) {
				...
				{	Ptable otbl = tbl;
					...
					tbl = std_tbl;
					if (vec_seen) {	/*  _vec_delete(vec,noe,sz,dtor); */
						int esz = cl->tsizeof();
...
//error('d',"ctor init=%d n_scope=%d",init,nn->n_scope);
				...
				case STATIC:
					sti = 1;
					if (tbl != gtbl) error('s',"local static%n of class with constructor",this);
				default:
					if (vec_seen && init) {
...
/*error('d',"dcl %t<-%t",nt,init->tp);*/
			...
			stgg:
				if (init && n_stclass== STATIC) {
					...
					switch (init->base) {
					case NAME:
						if (init->tp->tconst()==0) error("variable%n used inIr for%n",init,nn);
						break;
					case DEREF:
					case DOT:
					case REF:
