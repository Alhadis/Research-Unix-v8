...
*/
{
//error('d',"hide%n %d %k",this,this,base);
...
void end_cl()
{
	...
	if (modified_tn) {	// export nested class names to outer scope:
		...
		Plist local = 0;
		Plist nl = 0;
		for (Plist l=modified_tn; l; l=nl) {
			Plist nl = l->l;
			l->l = 0;
			Pname n = l->f;
//error('d',"%s%n",n->base==TNAME?"export":"local",n);
			if (n->base==TNAME && strcmp(n->string,ccl->string)) {
				if (export)
					export->l = l;
				else
					export = l;
...
//fprintf(stderr,"tname tn %s %d ll %d\n",tn->string,tn,tn->lex_level);
		Pname on = new name;
		tn->base = TNAME;
