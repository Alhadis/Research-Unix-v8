...
*/
{
	...
	while (t) {
		...
		case FIELD:
			tbuf->back(FIELD,t);
			tbuf->base( Pbase(t)->b_unsigned?uint_type:int_type );
			t = 0;
			break;
		case CLASS:
		case ENUM:
...
//error('d',"arg %d%k",arg,arg->base);
		...
		&& e->e2->base==ANAME) {
			...
			Pname a1 = (Pname)e->e1;
			Pname a2 = (Pname)e->e2;
/*error('d',"aname%n %d %d%n %d %d",a1,a1->n_val,a1->n_table,a2,a2->n_val,a2->n_table);*/
			Pexpr arg2 = aval(a2);
...
/*error('d',"val %d",val);*/
	...
	case FOR:
	{	int fi = for_init && (for_init->base!=SM || for_init->memtbl);
		if (fi) {
			puttok(LC);
			for_init->print();
		}
