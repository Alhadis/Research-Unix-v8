...
register struct iob *io;
{
	...
	io->i_offset = 0;
	io->i_bn = fsbtodb(io->i_blksz, itod(io->i_blksz, n)) + io->i_boff;
...
daddr_t bn;
{
	...
	for(; j<=3; j++) {
		if (blknos[j] != nb) {
			io->i_bn = fsbtodb(io->i_blksz, nb) + io->i_boff;
			io->i_ma = b[j];
			io->i_cc = BSIZE(io->i_blksz);
			devread(io);
			bap = (daddr_t *)b[j];
...
register struct iob *io;
{
	...
	while(n--) {
		if (++dc >= BSIZE(io->i_blksz)/sizeof(struct direct)) {
			io->i_bn = fsbtodb(io->i_blksz, sbmap(io, bn++)) + io->i_boff;
			io->i_ma = io->i_buf;
			io->i_cc = BSIZE(io->i_blksz);
			devread(io);
...
int	ptr;
{
	...
	if (fdesc < 0 || fdesc >= NFILES || ((io = &iob[fdesc])->i_flgs&F_ALLOC) == 0)
		return(-1);
	io->i_offset = addr;
	io->i_bn = fsbtodb(io->i_blksz, addr/BSIZE(io->i_blksz)) + io->i_boff;
	io->i_cc = 0;
	return(0);
}
...
int	fdesc;
{
	...
	if (io->i_cc <= 0) {
		...
		if (io->i_flgs&F_FILE)
			io->i_bn = fsbtodb(io->i_blksz,
				sbmap(io, dbtofsb(io->i_blksz, io->i_bn))) + io->i_boff;
		io->i_ma = io->i_buf;
		io->i_cc = BSIZE(io->i_blksz);
		devread(io);
		if (io->i_flgs&F_FILE) {
...
gotdev:
	*(cp-1) = '(';
	...
	file->i_blksz = devsw[file->i_ino.i_dev].dv_size;
	file->i_unit = *cp++ - '0';
	if (*cp >= '0' && *cp <= '9')
