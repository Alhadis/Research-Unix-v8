...
struct streamtab	nttyinfo = { &rinit, &winit };
	...
	register struct block	*b;
{
	...
	case M_DATA:
		...
		if (nt->nt_state & NT_ECHO && nt->nt_outq->next->flag & QDELIM)
			(void) putctl (nt->nt_outq, M_DELIM);
		return;
	case M_DELIM:
		freeb (b);
		return;
	...
	register struct block	*b;
{
	...
	case TIOCSETN:
		...
		(*q->next->qinfo->putp)(q->next, b);	/* send it on down */
		break;
