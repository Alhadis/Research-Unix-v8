...
#ifdef ETHERNET
	rminit(uhp->uh_map, NUBMREG-64, 1, "uba", UAMSIZ);
#endif ETHERNET
...
#ifdef ETHERNET
	if(numuba == 0) {		/*****EC ON UBA0 ONLY!!!*****/
		extern struct pte ECmap[];
		ecaccess((struct nexus *)(pumem-(24*1024)), ECmap);
	}
#endif ETHERNET
#if VAX780
	if (haveubasr) {
...
#define	ubaddr(off)	(u_short *)((int)vumem + ((off)&0x1fff))
...
#ifdef	CHAOS
		/*
		** Since the cold-boot-time stray interrupt catcher in
		** locore.s leaves the processor at the priority level
		** of the interrupt (yes, even after the rei), we must
		** restore it here.
		*/
		(void) spl0();
#endif	CHAOS
	...
	for (ui = ubdinit; udp = ui->ui_driver; ui++) {
		...
		reg = ubaddr(addr);
		if (badaddr((caddr_t)reg, 2)) {
			printf("%s%d not found at uba%d csr %o\n",
